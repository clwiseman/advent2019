...
func updatePath(path []coordinate, x int64, y int64) []coordinate {
	return append(path, coordinate{x, y})
}

func findPath(array [4]string) []coordinate {
	// Set up path
	var path []coordinate
	...
	// Loop through array and append coordinates
	for _, v := range array {
		x, y = calcCoordinate(x, y, v)
		path = updatePath(path, x, y)
	}
	return path
}

func manhattanDistance(x int64, y int64) int64 {
	return x + y
}

func findIntersection(array1 [4]string, array2 [4]string) int64 {
	path1 := findPath(array1)
	path2 := findPath(array2)

	var allPoints1 []coordinate
	var x, y int64
	for i := 1; i < len(path1); i++ {
		if (path1[i].X - path1[i-1].X != 0) {
			
			allPoints = updatePath(allPoints1, path1[i].Y)
		}
		if (path1[i].Y - path1[i-1].Y != 0)
	}

	var intersection coordinate

	return manhattanDistance(intersection.X, intersection.Y)
}

func main() {
	fmt.Println(findPath(test2))
}
